...
<body>
  ...
  <table>
    ...
    <tbody>
      <tr><td>Company Order</td><td>${amount}</td></tr>
    </tbody>
  </table>
</body>
</html>`;
    ...
    <div className="w-full mx-auto relative">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex justify-between items-center gap-3 w-full sm:w-auto">
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Company Orders</h1>
            ...
            <Table className="min-w-[800px]">
              <TableHeader className="sticky top-0 z-10 bg-yellow-50 border-b">
                <TableRow className="hover:bg-yellow-50">
                  ...
                  <TableHead className="min-w-40 bg-yellow-50">Employee Name</TableHead>
                      ...
                      <TableCell className="min-w-24">
                        <button className={`px-3 py-1.5 border w-28 rounded-xl ${
                          (order?.paymentStatus?.toUpperCase?.() === "PAID" || order?.paymentStatus?.toUpperCase?.() === "COMPLETED")
                            ? "border-green-200 text-green-600" : "border-red-200 text-red-600"}`}>{order?.paymentStatus}</button>
                      </TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No Orders found matching your search.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
